...
function _readStagePilotPerfEvidenceArtifact() {
  const fallback = {
...
function _readStagePilotTraceObservabilityArtifact() {
  const fallback = {
    generatedAt: null,
    hotspots: [] as Array<{
...
function _readStagePilotRegressionGateArtifact() {
  const fallback = {
    generatedAt: null,
    gates: [] as {
      decision: string;
...
function buildBenchmarkSummaryPayload(rawUrl?: string): JsonObject {
  const parsed = new URL(rawUrl ?? "/", "http://127.0.0.1");
  const strategy = parsed.searchParams.get("strategy");
  if (
    strategy &&
    !["baseline", "middleware", "middleware+ralph-loop"].includes(
  ...
  const minSuccessRateRaw = parsed.searchParams.get("minSuccessRate");
  const minSuccessRate =
    minSuccessRateRaw && Number.isFinite(Number(minSuccessRateRaw))
      ? Number(minSuccessRateRaw)
      : null;
...
function buildDeveloperOpsPackPayload(rawUrl?: string): JsonObject {
  const laneConfig = parseStagePilotLane(rawUrl);
  if (laneConfig.error) {
    throw new HttpError(400, laneConfig.error);
  }
...
function buildPerfEvidencePackPayload(): JsonObject {
  return buildStagePilotPerfEvidencePack({
    benchmarkSnapshot: readStagePilotBenchmarkSnapshot(),
    perfArtifact: _readStagePilotPerfEvidenceArtifact(),
    service: process.env.SERVICE_NAME_API ?? "stagepilot-api",
  });
}
...
function buildTraceObservabilityPackPayload(): JsonObject {
  return buildStagePilotTraceObservabilityPack({
    benchmarkSnapshot: readStagePilotBenchmarkSnapshot(),
    service: process.env.SERVICE_NAME_API ?? "stagepilot-api",
    traceArtifact: _readStagePilotTraceObservabilityArtifact(),
  });
}
...
function buildRegressionGatePackPayload(): JsonObject {
  return buildStagePilotRegressionGatePack({
    benchmarkSnapshot: readStagePilotBenchmarkSnapshot(),
    regressionArtifact: _readStagePilotRegressionGateArtifact(),
    service: process.env.SERVICE_NAME_API ?? "stagepilot-api",
  });
}
...
function handleBenchmarkSummaryRequest(
  ...
  try {
    sendJson(response, 200, buildBenchmarkSummaryPayload(rawUrl), options);
  } catch (error) {
    const httpError = toHttpError(error);
    sendJson(
      response,
      httpError.statusCode,
      { error: httpError.message, ok: false },
      options
    );
  }
}
...
function handleDeveloperOpsPackRequest(
  ...
  try {
    sendJson(response, 200, buildDeveloperOpsPackPayload(rawUrl), options);
  } catch (error) {
    const httpError = toHttpError(error);
    sendJson(
      response,
...
function handleWorkflowReplayRequest(
  response: ServerResponse,
  rawUrl?: string,
  options?: { includeBody?: boolean }
) {
  ...
  sendJson(
    ...
    buildStagePilotWorkflowReplay({
      lane:
        laneConfig.lane === "merge-request" ||
        laneConfig.lane === "pipeline-recovery" ||
        laneConfig.lane === "release-governor"
          ? laneConfig.lane
          : undefined,
    }),
    options
...
function handleReadonlyRequest(options: {
  ...
  switch (pathname) {
    ...
    case "/v1/developer-ops-pack":
      handleDeveloperOpsPackRequest(response, rawUrl, { includeBody });
      return true;
    ...
    case "/v1/workflow-run-replay":
      handleWorkflowReplayRequest(response, rawUrl, { includeBody });
      return true;
