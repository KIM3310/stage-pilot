...
function readArtifactSummary(
  ...
  const parsedForensics = parseForensicsPayload(
    readJsonIfExists(errorForensicsPath) as Record<string, unknown> | null
  );
  ...
  return {
    ...
    dominantBucket: parsedForensics.dominantBucket,
    updatedAt,
...
export function createBenchLabApiServer(
  ...
  function buildSummaryPackPayload(): JsonObject {
    const artifacts = listArtifactSummaries(repoRoot);
    return buildBenchLabSummaryPack({
      artifactCount: artifacts.length,
      artifactsWithTrackedErrors: artifacts.filter(
        (item) =>
          typeof item.dominantBucket === "string" &&
          item.dominantBucket.length > 0
      ).length,
      bestArtifactClaimName:
        artifacts.find((item) => typeof item.claimName === "string")
          ?.claimName ?? null,
      bestArtifactDeltaPp:
        artifacts.find((item) => typeof item.deltaPp === "number")?.deltaPp ??
        null,
      configCount: listConfigFiles(matrixRoot).length,
      dominantBucket:
        artifacts.find((item) => typeof item.dominantBucket === "string")
          ?.dominantBucket ?? null,
      improvedArtifactCount: artifacts.filter(
        (item) => typeof item.deltaPp === "number" && item.deltaPp > 0
      ).length,
      jobCount: jobs.size,
      runCount: listRuntimeSummaries(matrixRoot).length,
    });
