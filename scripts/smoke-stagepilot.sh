...
resource_pack_json="$(curl_json "${BASE_URL}/v1/architecture-resource-pack")"
resource_pack_ok="$(echo "${resource_pack_json}" | jq -r '.schema')"
if [[ "${resource_pack_ok}" != "stagepilot-architecture-resource-pack-v1" ]]; then
  echo "[smoke-stagepilot] architecture-resource-pack check failed: ${resource_pack_json}" >&2
  exit 1
fi
echo "[smoke-stagepilot] architecture-resource-pack ok"
