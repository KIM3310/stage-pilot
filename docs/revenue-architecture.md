# Revenue Architecture - stage-pilot
...
| Layer | Decision |
| --- | --- |
| Target buyer / user | AI product team that needs reliable tool-calling tests before shipping non-native or mixed-provider agents |
| Productized offer | tool-calling reliability gateway with regression tests, provider scorecards, and trace observability |
| First paid SKU | paid hosted regression workspace, private benchmark scenarios, and provider routing dashboard |
| Free lead magnet | free public benchmark/demo routes and sample traces |
| Paid expansion | scenario credits, hosted trace retention, and team API key vault |
| Data / workflow moat | tool-call failure corpus, parser recovery metrics, provider scorecards, and regression history |
| Private inquiry | https://kim3310-doeon-kim-portfolio.pages.dev/?offer=stage-pilot&inquiry=agent-reliability-audit#private-inquiry |
...
| Repo-specific launch path | Cloudflare Pages front, Workers/Node API, D1 run metadata, R2 trace bundles, OpenRouter/Groq/Cerebras/NIM provider adapters |
...
2. Route confidential and commercial requests through the [central Cloudflare D1 private inquiry](https://kim3310-doeon-kim-portfolio.pages.dev/?offer=stage-pilot&inquiry=agent-reliability-audit#private-inquiry); keep public GitHub issues limited to non-confidential product discussion.
